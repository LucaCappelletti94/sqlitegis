...
use sqlitegis::diesel::types::{Geography, Geometry};
...
fn vec_u8_roundtrip_geometry() {
    ...
    let ewkb =
        sqlitegis::core::ewkb::write_ewkb(&geo::Geometry::Point(geo::Point::new(1.0, 2.0)), None)
            .unwrap();
...
fn vec_u8_roundtrip_geography() {
    ...
    let ewkb = sqlitegis::core::ewkb::write_ewkb(
...
fn geo_geography_roundtrip_has_srid() {
    ...
    let blob = row.geom.expect("geom should not be NULL");
    let (_geom, srid) = sqlitegis::core::ewkb::parse_ewkb(&blob).unwrap();
...
fn geography_fromsql_rejects_missing_srid() {
    let mut c = conn();
    let ewkb =
        sqlitegis::core::ewkb::write_ewkb(&geo::Geometry::Point(geo::Point::new(13.4, 52.5)), None)
            .unwrap();
...
fn slice_tosql() {
    ...
    let ewkb =
        sqlitegis::core::ewkb::write_ewkb(&geo::Geometry::Point(geo::Point::new(9.0, 10.0)), None)
            .unwrap();
