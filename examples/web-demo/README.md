...
- `src/db.rs`. `Once::call_once` plus `sqlite_wasm_rs::sqlite3_auto_extension` registers `sqlitegis::sqlite::register_functions` for every new connection, then opens a single `SqliteConnection::establish(":memory:")` shared across the page.
